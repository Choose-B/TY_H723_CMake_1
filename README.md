...
> 这个严格意义上是打别的比赛用的，但是工程本身是一个配置好的工程，故整理发出
...
1. 修改成适合自己电脑的一些配置文件
2. 严格按照四层结构去写，然后写完整的doxygen注释 （每个人的代码都有自己的小习惯 在每个文件开头注明author）
...
其他文件功能若需转接就写`api_xxxx.cpp`和`api_xxx.h` 作为区分 **`h`**

正常文件为：`xxx_yyyy.cpp` 和 `xxx_yyyy.hpp` 作为区分 **`hpp`**
